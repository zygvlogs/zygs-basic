...
Press `Ctrl+C` (or your terminal's break sequence) to stop execution if necessary (Note: the current interpreter loop checks `Serial.available`, so sending any key might interrupt certain loops depending on implementation).
...
This program is free software: you can redistribute it and/or modify it under the terms of the **GNU General Public License v3.0**. See the [LICENSE](https://www.gnu.org/licenses/gpl-3.0.html) for more details.
